# Sberbank Online

1. Site: online.sberbank.ru
2. Current datetime: 21.09.2015 22:53 Msk
3. Browsed URLs: https://online.sberbank.ru/CSAFront/index.do
4. Research:
```
    IP: 194.54.14.131
    HTTPS cert: 
        Owner: Sberbank of Russia OJSC
        Verified: thawte, Inc.
        Expires: 27.12.2016
        Vers: 3
        Algorythm: PKCS #1 SHA-256 With RSA Encryption
        CN = online.sberbank.ru
        OU = 00CA
        O = Sberbank of Russia OJSC
        L = Moscow
        ST = Moscow
        C = RU
        Object Identifier (2 5 4 5) = 1027700132195
        Object Identifier (2 5 4 15) = Private Organization
        Object Identifier (1 3 6 1 4 1 311 60 2 1 3) = RU
    WEB:
        redirect from http://online.sberbank.ru to https://online.sberbank.ru/CSAFront/index.do
        Server response:
            HTTP/1.1 200 OK
            X-Powered-By	Servlet/3.0
            Content-Type	text/html;charset=windows-1251
            X-Frame-Options	SAMEORIGIN
            X-Client-IP	136.243.98.54
        Cookies:
            JSESSIONID	0000AauViclLfIlYaAtYG4rrQGo:18rc1dmto|rs DPJSESSIONID=PBC5YS:881931240	/CSAFront/	.online.sberbank.ru	End Of Session
            cfids	QkQ7FNLy4Y9SESk3fHCi39XKHFXC7Lx6zP8IK8jBA7qkGP9Ioqk8GG95gDg%2BvvisTybbss9QNJELLToWdO6EoQ%3D%3D	/	online.sberbank.ru	Tuesday, September 20, 2016 19:51:32
        Comments:
            Static image files lives on separate server: https://stat.online.sberbank.ru/CSAFront-res/16.1/skins/sbrf/images/favicon.ico
            There are T-Party scripts from:
                https://ibjs.group-ib.ru/vaultonline-2.js
                https://www.googletagmanager.com/gtm.js?id=GTM-NRDX7Z
                https://cdn.rutarget.ru/static/tag/tag.js
                https://ssl.google-analytics.com/ga.js
                https://mc.yandex.ru/metrika/watch.js
            There are ajax content from:
                https://online.sberbank.ru/CSAFront/async/news.do
            Index page code analyse:
                HTML5
                Login controller: /CSAFront/login.do
                Login recovery: /CSAFront/page/login-error.do
                Registration: /CSAFront/async/page/registration.do
            DO-files: Web-based Java program run by a Web server that supports Java, such as Tomcat or IBM WebSphere; 
                typically mapped to the Struts controller, which processes the file; used for generating dynamic Web pages.
                
```
5. 